...
import { getDashboardSummary, getDeals, getScanSummary } from "@/lib/services/deals";
...
import { ScanTrigger } from "@/components/monitoring/scan-trigger";
import { ScanSummaryCard } from "@/components/monitoring/scan-summary";
...
export default async function DashboardPage() {
  ...
  const [summary, topDeals, profitDeals, preferences] = await Promise.all([
    getDashboardSummary(context.dealerId),
    getDeals(context.dealerId, { sort: "score_desc" }),
    getDeals(context.dealerId, { sort: "profit_desc" }),
    getDealerPreferences(context.dealerId)
  ]);
  const scanSummary = await getScanSummary(context.dealerId, preferences);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card className="px-4 py-3">
        <ScanSummaryCard summary={scanSummary} />
        <ScanTrigger intensity={preferences.monitoringIntensity} />
      </Card>
